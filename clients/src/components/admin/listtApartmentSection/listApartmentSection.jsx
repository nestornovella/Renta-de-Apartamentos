...
function ListApartmentSection() {
  ...
  const { apartments, resetData, getApartments, getAnApartment } =
    useAdminApartments();
  const { detail, getDetail, resetDetail, deleteApartment } =
    useAdeminApartDetail();
  useEffect(() => {
    getApartments();
  }, []);
  ...
  return (
    <dir className="grid h-[60%] md:grid-cols-1 xl:grid-cols-2 gap-2 p-0  ">
      {edit && detail ? (
        <EditApartment sendInput={setHookState} detail={detail} />
      ) : (
        <RentSection
          detail={detail}
          setEdit={setEdit}

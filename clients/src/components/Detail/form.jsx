...
function Form({ apartmentId, availability, urbanizacion, status, rentalType }) {
  ...
  function reserveRent() {
    if (availability) {
      payment(rentalType)
      alertTopPaypal('You will be redirected to the Bold platform.','warning', 10000)
...

          {
            ...
            <div className="shadow-2xl border rounded-lg my-2 p-2">
              <p className="text-xs text-gray-400 mt-2">Do you want to reserve an apartment now?</p>
               <div onClick={reserveRent} className={`relative flex justify-center items-center text-white gap-2 font-semibold cursor-pointer ${availability ? 'hover:text-blue-500' : 'hover:text-red-500'} transition-all delay-200 hover:bg-black  bg-gradientBold py-2 px-3 hover:black rounded-xl mt-2`}>
                {availability ? <span className="text-[15px] text-white text-semibold ">Reserve with Bold</span> : <span className="text-[15px] text-white text-semibold ">Not Availability</span>}
                {availability ? <img className="w-[50px]" src={BoldLogo}/>: <FaCalendarXmark />}
              </div>
            </div>
          }
